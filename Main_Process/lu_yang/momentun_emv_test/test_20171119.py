...
@file: test_20171119.py
@time: 2017/11/19 20:30
"""
"""
对单个股票进行emv, sma模拟
...
# ====生成动量函数的结果
output = momentum(df_monthly)
output['equity'] = (output['change'] + 1).cumprod()
output['nat_equity'] = (output['nat_equity'] + 1).cumprod()
output['index_change'] = (output['index_change'] + 1).cumprod()
output.to_csv(Functions.out_put_path + '/Going_Merry/range_test/'+'600000sma1_momentum.csv')
...
plt.plot(output['equity'], label='momentum')
