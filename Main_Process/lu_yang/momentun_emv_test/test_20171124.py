...
@file: test_20171124.py
@time: 2017/11/24 21:21
"""
...
# stock_data = pd.read_hdf('EMV600000.h5', key='600000')
# stock_data.to_csv('EMV600000.csv')
# exit()
...
def momentum_contrarian(df, type='momentum', window=3):
    ...
    if type == 'momentum':
        for i in range(window, len(df)-1):  # 因为行是从0开始所以要减1
            ...
            param = grouped.index[-1]  # 在收益率相等的时候取最大的参数
            # print grouped
            # print param
            # print df.at[i, 'date'], df.at[i, param]
            momentum_output.at[i - window, 'date'] = df.at[i, 'date']
            momentum_output.at[i - window, 'change'] = df.at[i, param]
            momentum_output.at[i - window, 'param'] = param
        momentum_output = pd.merge(right = momentum_output, left= df_index_nat, on = 'date', how='inner')
        ...
        for i in range(window, len(df) - 1):  # 因为行是从0开始所以要减1
            ...
            param = grouped.index[1]  # 在收益率相等的时候取最小的参数
            # print grouped
            # print param
            # print df.at[i, 'date'], df.at[i, param]
            contrarian_output.at[i - window, 'date'] = df.at[i, 'date']
            contrarian_output.at[i - window, 'change'] = df.at[i, param]
            contrarian_output.at[i - window, 'param'] = param
        contrarian_output = pd.merge(right=momentum_output, left=df_index_nat, on='date', how='inner')
        return momentum_output
