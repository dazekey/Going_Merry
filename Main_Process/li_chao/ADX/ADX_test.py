...
for code in stock_code_list[0:]:    # 可以适当少选一点股票
    ...
    stock_data = stock_data[['date', 'code', 'open', 'high', 'low', 'close', 'change']]
    ...
    for n in range(14, 15):
        df = TA_strategy.adx(stock_data, n)
        ...
        df = df[['date', 'code', 'open', 'high', 'low', 'close', 'change', 'equity']]
        # print df
        # exit()
    ...
    re.iloc[0:1, :].to_csv('C:/all_trading_data/data/output_data/adx_test_20171110.csv', mode='a', header=None, index=False)